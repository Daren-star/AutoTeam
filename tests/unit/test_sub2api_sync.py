...
def test_build_extra_records_configured_instance_id(monkeypatch):
    monkeypatch.setattr(sub2api_sync, "AUTOTEAM_INSTANCE_ID", "team-a", raising=False)
...
def test_is_managed_account_filters_by_current_instance(monkeypatch):
    ...
    assert sub2api_sync._is_managed_account(
        {"extra": {**base_extra, "autoteam_instance_id": "team-a"}}, kind="pool"
    )
    assert not sub2api_sync._is_managed_account(
        {"extra": {**base_extra, "autoteam_instance_id": "team-b"}}, kind="pool"
    )
    assert not sub2api_sync._is_managed_account({"extra": base_extra}, kind="pool")

    monkeypatch.setattr(sub2api_sync, "AUTOTEAM_INSTANCE_ID", "", raising=False)
    assert sub2api_sync._is_managed_account({"extra": base_extra}, kind="pool")
