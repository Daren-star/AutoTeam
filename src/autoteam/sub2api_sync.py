...
def _has_managed_marker(item: dict) -> bool:
    extra = item.get("extra") or {}
    if not isinstance(extra, dict):
        return False
    return extra.get(_EXTRA_SOURCE) == "autoteam" or bool(extra.get(_EXTRA_MANAGED))
...
def _is_managed_account(item: dict, *, kind: str | None = None, instance_id: str | None = None) -> bool:
    ...
    if not isinstance(extra, dict):
        return False
    if not _has_managed_marker(item):
        return False
...
def _find_managed_account_by_email(items: list[dict], email: str, *, kind: str | None = None) -> dict | None:
    ...
    for item in items:
        ...
        if kind is None or _is_managed_account(item, kind=kind):
            matches.append(item)
        elif not _has_managed_marker(item):
